...
simulation model ('MixFishSim`) incorporating: i) delay-difference population
...
\deleted{Fifty years of sub-daily fishing activity was simulated and used to
	...
	applied the framework to draw}\deleted{compared} inference\deleted{s}
\added{on community structure} \added{when using data generated from
}\deleted{based on}: commercial catch, a simulated fixed-site sampling survey
design and the true \added{(simulated)} underlying populations.
\deleted{W}\added{In doing so w}e \deleted{i)}establish the potential
limitations of fishery-dependent data in providing a robust picture of
spatiotemporal distributions\deleted{; and then ii) simulated an area closure
	...
	different populations w} \added{W}e found different spatial patterns
were evident and that the effectiveness of the spatial closure reduced when
data were aggregated across larger spatial areas. \deleted{; and then
	ii)}\added{We then simulated an area closure based on areas defined
