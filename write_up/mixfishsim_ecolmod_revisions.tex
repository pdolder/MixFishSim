...
simulate patchy, heterogeneously distributed \added{and moving fish}
populations, and iii) fishery dynamics for multiple fleet characteristics based
on population targeting under an explore-exploit strategy.\deleted{ This is
	implemented via a mix of correlated random walk movement (for
	exploration) and learned behaviour (for exploitation) phases of the
	fisheries. }\\ 
%%
\deleted{Fifty years of sub-daily fishing activity was simulated and used to
	draw inference on the underlying community structures. W}\added{We
	applied \emph{MixFishSim} to\deleted{draw}}\deleted{compared}
infer\deleted{ence}\deleted{s} \added{\deleted{on}community structure}
\added{when using data generated from:} \deleted{based on}commercial catch,
a\deleted{simulated} fixed-site sampling survey design and the true
\added{(simulated)} underlying populations.  \deleted{W}\deleted{\added{In
		doing so w}}\added{W}e \added{thereby} \deleted{i)}establish
the potential limitations of fishery-dependent data in providing a robust
\replaced{characterisation}{picture} of spatiotemporal distributions.\deleted{;
	and then ii) simulated an area closure based on areas defined from the
	different data sources at a range of temporal and spatial
	resolutions.}\deleted{Our framework allows users to explore the
	assumptions in modelling observational data and evaluate the underlying
	dynamics of such approaches at a fine spatial and temporal scale. In
	application to a mixed fishery exploiting four different populations w}
\deleted{\added{W}e found d}\added{D}ifferent spatial patterns were evident and
\deleted{that }the effectiveness of the spatial closure reduced when data were
aggregated across larger spatial areas. \deleted{; and then
	ii)}\deleted{\added{We then simulated an area closure based on areas
		defined from the different data sources at a range of temporal
		and spatial resolutions.}} \added{A simulated area
	closure showed that} \replaced{a}{A}ggregation across time periods has
less of a negative impact on the closure success than \added{aggregation} over
space. \deleted{ and w}\added{W}hile not as effective as when based on on the
true population, closures based on high catch rates observed in commercial data
were still able to reduce fishing on a protected species. \\ 

\added{Our framework allows users to explore the assumptions in modelling
	observational data and evaluate the underlying dynamics of such
	approaches at a fine spatial and temporal resolutions. From our
	application w}\deleted{W}e conclude that commercial data, while
...
closures as measures to reduce unwanted catch\added{ which is often discarded}
\citep{Kennelly2002, Catchpole2008, Bellido2011, Cosgrove2019}\added{.
	A}\deleted{ and a}daptive spatial management strategies have been
...
Identifying appropriate spatial scales for fisheries closures \deleted{has been a
highlighted a}\added{i}s crucial to their success \citep{Costello2010, Dunn2016}.
...
closures \replaced{that}{which} are ineffectual or have unintended consequences. For example,
...
\begin{enumerate}
	\item Do different source\added{s} of sampling-derived fisheries data
		reflect\deleted{s} the underlying community structure? 	
	\item How do\deleted{es} data aggregation and data source impact on the success
...
appropriate to capture \deleted{the }characteristic\added{s} of the different processes (Figure
1). \replaced{S}{The following s}ub-modules \deleted{were included }to capture the full system \added{comprised}: 1)
\replaced{p}{P}opulation dynamics, 2) \replaced{r}{R}ecruitment dynamics, 3) \replaced{p}{P}opulation movement, 4)
fishery dynamics.\\

Population dynamics \added{for any number of species, as chosen by the user,}
operate on a daily time-step \added{(with recruitment occurring only during
	defined seasons for each population)}, while population movement occurs
...
Deriso-Schnute delay difference model \replaced{that}{which} models the fish populations in
...
follows (\replaced{see}{the full parameter list is detailed in } Table \ref{tab: pop_variable} \added{for all parameter details}): 
\begin{equation}
	\begin{split}
	B_{c,d+1} = &\\
	...
	\end{split}
\end{equation}
where $\rho$ is Brody's coefficient\textcolor{blue}{(Check)}, shown to be equal
to $\mathrm{e}^{-K}$ when $K$ is the \deleted{growth }rate \added{at which the
	asymptote is approached} from a von Bertalanffy \deleted{logistic
}growth model \citep{Schnute1985}. $W_{R-1}$ is the average weight of fish
prior to recruitment, while $W_{R}$ is the average recruited weight.
$\alpha_{d}$ represents the proportion of fish recruited during that day for
the year, while $R_{c,\tilde{y}(c)}$ is the annual recruits in year $y$ for
cell $c$. \\
...
\begin{equation}
	\begin{split}
	\bar{R}_{c,d} = & \frac{(\alpha \deleted{\cdot} S_{c,d})}{(\beta + S_{c,d})} \\
...
\added{Stochastic probabilities are affected by the suitability of habitat,
	temperature in a cell and the thermal tolerance of a population to that
	temperature}.\\

\added{The combined process results in a population structure and movement
	pattern unique to each population, with population movement occurring
	on a weekly basis. \replaced{M}{The decision to m}odel\added{ing}
	population movement on a weekly timescale\deleted{ was to}
	reflect\added{s} that fish tend to aggregate in
	species\added{-}specific locations\deleted{ that have been} observed to
	last \replaced{between}{around } one \replaced{and}{to} two weeks
	\citep{Poos2007}. Therefore this process approximated the demographic
	shifts in fish populations throughout a year with seasonal spawning
	patterns }(Figure \ref{S-fig:5}).
...
determines the smoothness of the process. \added{We use the most commonly used
	Matérn covariance structure as it is a flexible form that \replaced{contains the}{under certain
	conditions is of the same form as an} exponential \added{and double exponential as special cases}\deleted{ function} and
	it}\deleted{This} enables us to model the spatial autocorrelation
observed in animal populations where density is more similar in nearby
locations\deleted{, \added{but that correlation decreases non-linearly}}
...
A time-varying temperature covariate changes the \deleted{interaction between time and}
...
each cell and population temperature tolerance is defined \replaced{as}{that}:
\begin{equation}
...
tolerance \replaced{(see Table \ref{tab:pop_move} for variable descriptions).}{. The variables, their meaning and units for population movement is
provided in Table \ref{tab:pop_move}}. \\

\deleted{The final combined process results in a population structure and movement
...
been observed to last around one to two weeks (Poos et al 2007). Therefore this
...
Fleet dynamics \replaced{were}{can be }broadly categorised into three
components. \textit{Fleet targeting} \replaced{determined}{determines} the
fleet catch efficiency and preference towards a particular population;
\textit{trip-level decisions} \replaced{determined}{that determines} the
initial location to be fished at the beginning of a trip; and
\textit{within-trip decisions} \replaced{determined fishing locations}{, that
	determines movement from one fishing spot to another} within a trip.
\replaced{This results in an explore-exploit strategy}{ was implemented in the
	model that combined these three components} for individual vessels to
...
population-specific movement rates (Table \ref{tab:1}).  \added{The population
...
dynamics hypothetical but \deleted{as you} might \added{be} expect\added{ed} \deleted{to find }in a typical demersal
fishery.}

\subsection{Fleet calibration}

\replaced{F}{The f}leets were calibrated to reflect five different characteristic fisheries
...
preferences between the fleets and hence \added{different} spatial dynamics.  The learned random
...
function with a lower asymptote of 0 and upper asymptote of 0.95, and a \replaced{slope}{growth
...
per day) with same catchability parameter ($Q_{p} = 1$) for all populations\deleted{ $p$}.
...
\added{We implemented a series of spatial closures targeted at reducing fishing
	mortality on population 3, given the different data sources and spatial
	and temporal resolutions above. We use the effectiveness of these
	closures in reducing fishing mortality as a way of evaluating the
	trade-offs in data sources and resolution.} Survey closures were on an
...
\deleted{The }\deleted{aggregated }\replaced{C}{c}atch composition \added{aggregated at different
	spatial resolutions} from each of the data sources (\deleted{which
	shows }average seasonal patterns over a ten-year period) highlights
...
sampling domain). \replaced{S}{The s}urvey data at this spatial resolution displays very
...
\replaced{Weekly}{The} commercial data \deleted{on a weekly basis }shows some
of the same patterns as the true population, though \deleted{the }population 1
\deleted{(in red)} is less well represented and some weeks are missing catches
from the area. Here, weekly and monthly compositions were nearly identical
(Figure \ref{fig:2}; Table \ref{tab:7}). \deleted{Again, y}\added{Y}early
values h\deleted{e}ad a similar mean but smaller standard deviation. The survey
data was only available on an annual basis, and showed again a slightly
different composition from the true population and the commercial data; in
particular a greater proportion of population 4 (Figure \ref{fig:2}).

\subsection{How does data aggregation and source impact on spatial fisheries
	management measures?}

\deleted{We implemented a spatial closure using the different data sources and
	spatial and temporal aggregations as outlined in the protocol in
	Section 3.4. We used this to assess the efficacy of a closure in
	reducing fishing mortality on population 3, given availability of data
	and its use at different resolutions in order to evaluate the
	trade-offs in data sources. }
...
closures, following which the fisheries ``adapted" to the closures \added{by
	finding new areas of high abundance to fish. This led to} fishing
...
effective for population 3), followed by \added{spatial} data resolution (21 \%), data type (7
...
allows users to explore \deleted{the} assumptions in modelling observational
data and \added{to} evaluate the underlying dynamics of such approaches at
\deleted{a} fine spatial and temporal scale\added{s}.  This offers the
...
fleets exploiting multiple fish populations\added{. This shows how}\deleted{
	and} realis\added{ed}\deleted{ing} catch distributions \deleted{that
}may differ from the underlying populations, as identified by
...
fished. Similarly, despite \deleted{the }even spatial coverage the survey \deleted{was able to
}capture\added{d} some of the same spatial patterns as the true population, but missed
...
experienced in \replaced{given}{individual} locations over the course of the year (Figure
\ref{S-fig:9}). \\ 
...
commercially derived data have the potential to reduce\deleted{d} fishing
...
opportunities for other fisheries (Figure \ref{fig:ce}). \\
...
the importance \replaced{of}{in} considering fishing effort reallocation following spatial
...
locational information from commercial fisheries will \replaced{make}{require} it \deleted{to be }a key
...
that the features can be distinguished. We found \deleted{in our application
}greater spatial than temporal heterogeneity. When using aggregated data to
define spatial closures coarser temporal resolution (months instead of weeks)
could still achieve the same results in reducing exploitation rates of a
vulnerable species at the highest temporal resolution data. Conversely,
reducing the spatial resolution had a negative effect on the effectiveness of
the measures \deleted{(}though\added{,} importantly, there was still some
benefit even with coarse spatial resolution\deleted{)}. \\

While \deleted{any findings are likely to be} case\added{-}specific, our
findings emphasise the need to understand population demographics, habitat use
and movement rates in designing any closure scenario based on observational
sampling. This information can then be used to set the bounds on data
aggregation used in modelling studies aimed at informing the management
measures. \\ 

\deleted{\textit{MixFishSim} has numerous potential additional applications as it
...
hypothes\replaced{e}{i}s that are otherwise unable to be analysed using real
...
Fisheries and Aquaculture Science seedcorn program (DP227AC).\added{ We thank
...
\begin{figure}[!ht]
	...
	\caption{[Colour] Data aggregation at different spatial resolutions
		over a ten year period. \added{The figure shows catch
