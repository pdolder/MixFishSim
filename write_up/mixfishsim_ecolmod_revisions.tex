...
\usepackage[draft]{changes}%\usepackage[draft]{changes}
...
simulation model (`MixFishSim')\textcolor{blue}{[CM: I think \emph{MixFishSim} is more effective]} incorporating: i) delay-difference population
dynamics, ii) population movement using Gaussian Random Fields to simulate
patchy, heterogeneously distributed \added{and moving fish} populations, and
iii) fishery dynamics for multiple fleet characteristics based on population
targeting under an explore-exploit strategy.\deleted{ This is implemented via a mix of
correlated random walk movement (for exploration) and learned behaviour (for
exploitation) phases of the fisheries. }\\ 
%%
\deleted{Fifty years of sub-daily fishing activity was simulated and used to
	...
	applied `MixFishSim' to\deleted{draw}}\deleted{compared} infer\deleted{ence}\deleted{s}
\added{\deleted{on}community structure} \added{when using data generated from:}
\deleted{based on}commercial catch, a\deleted{simulated} fixed-site sampling survey
design and the true \added{(simulated)} underlying populations.
\deleted{W}\deleted{\added{In doing so w}}\added{W}e \added{thereby} \deleted{i)}establish the potential
limitations of fishery-dependent data in providing a robust \replaced{characterisation}{picture} of
spatiotemporal distributions.\deleted{; and then ii) simulated an area closure
	based on areas defined from the different data sources at a range of
	temporal and spatial resolutions.}\deleted{Our framework allows users
	to explore the assumptions in modelling observational data and evaluate
	the underlying dynamics of such approaches at a fine spatial and
	temporal scale. In application to a mixed fishery exploiting four
	different populations w} \deleted{\added{W}e found d}\added{D}ifferent spatial patterns
were evident and \deleted{that }the effectiveness of the spatial closure reduced when
data were aggregated across larger spatial areas. \deleted{; and then
	ii)}\deleted{\added{We then simulated an area closure based on areas defined
	from the different data sources at a range of temporal and spatial
<<<<<<< HEAD
	resolutions.}} \added{A simulated area closure showed that} \replaced{a}{A}ggregation across time periods has less of a negative
impact on the closure success than \added{aggregation} over space. \deleted{ and w}\added{W}hile not
...
\added{\replaced{MixFishSim}{Our framework} allows users to explore the assumptions in modelling
=======
	resolutions.} Aggregation across time periods has less of a negative
impact on the closure success than over space. \deleted{ and w}\added{W}hile not
as effective as when based on on the true population, closures based on high
catch rates observed in commercial data were still able to reduce fishing on a
protected species. \\ 
%%
\added{Our framework allows users to explore the assumptions in modelling
>>>>>>> f5e5489e3bc3778abecffc1d0c3d90fdb3592e8b
	observational data and evaluate the underlying dynamics of such
	approaches at a fine spatial and temporal scale. From our application
	w}\deleted{W}e conclude that commercial data, while containing bias,
...
\begin{equation}
	...
	\hspace{0.9cm} + \\
	& Wt_{R} \cdot \left( \alpha_{d} \cdot R_{\tilde{y} \left(c\right)}
	\right) \\
	\end{split}
\end{equation}
where $\rho$ is Brody's coefficient, shown to be equal to $\mathrm{e}^{-K}$
when $K$ is the \deleted{growth }rate \added{at which the asymptote is approached} from a von Bertalanffy \deleted{logistic }growth model
...
\added{Stochastic probabilities are affected by the suitability of habitat, temperature in a cell
...
basis. \replaced{M}{The decision to m}odel\added{ing} population movement on a weekly timescale\deleted{ was to}
reflect\added{s} that fish tend to aggregate in species\added{-}specific locations\deleted{ that have
been} observed to last \replaced{between}{around } one \replaced{and}{to} two weeks \citep{Poos2007}. Therefore this
process approximated the demographic shifts in fish populations throughout a
year with seasonal spawning patterns }(Figure \ref{S-fig:5})\textcolor{blue}{[CM: shouldn't it be S1 so that the they follow the main ms? Maybe not \dots]}.
...
\replaced{Weekly}{The} commercial data \deleted{on a weekly basis }shows some of the same patterns as the
true population, though \deleted{the }population 1 \deleted{(in red)} is less well
...
\ref{tab:7}). \deleted{Again, y}\added{Y}early values h\deleted{e}ad a similar mean but
...
data have the potential to reduce\deleted{d} fishing mortality. The likely cost
...
that the features can be distinguished. We found \deleted{in our application }that there
...
While \deleted{any findings are likely to be} case\added{-}specific, our findings emphasise the
