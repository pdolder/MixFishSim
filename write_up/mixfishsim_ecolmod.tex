...
\replaced[id=PD]{M}{Since then
	m}ore refined spatial information has \added[id=PD]{since} become
...
\begin{enumerate}
	\item How does sampling-derived data reflects the underlying community 
		structure? 	
...
or commercial catch\added[id=PD]{, we can use the population model to validate
	...
	sampling relates to the real community structure in a way we could not
...
\deleted[id=PD]{Here,}Population
biomass growth and depletion for pre-recruits and\deleted[id=PD]{fish}
recruited \added[id=PD]{fish}\deleted[id=PD]{to the fishery} were modelled
