...
\replaced[id=PD]{as defined by the expected profit}{in value}. 
...
thermal tolerances results different spatial habitat use for each population
...
exploits four different fishing grounds, three of them multiple time (Figure
\ref{S-fig:11}), while across multiple trips fishing grounds that are further
...
\added[id=PD]{Figure} \ref{S-fig:13}, which shows several trips for the vessel
overlaid on the value field (sum of the population densities $\times$ price).
...
\ref{S-fig:14}), which is the result of the correlated random walk step
function, with distance moved during the exploitation phase related to the
revenue experienced on the fishing ground (\replaced[id=PD]{Figure}{while}
\ref{S-fig:15}).\todo{\added[id=JJ]{Move some of the
