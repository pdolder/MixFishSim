...
spatiotemporal distributions. [We simulate a closure based on areas defined
from commercial catch data and assess its effectiveness on reducing catches of
a fish population.] \\
...
i. Extract data source
ii. Aggregate according to resolution
iii. Interpolate across entire area at desired resoltion
iv. Close top 5 \% of areas

\begin{table}[!ht]
\caption{Closure scenarios}
\begin{tabular}{ p{4cm } p{4cm} p{4cm}}
	\hline
	Data type & Temporal resolution & Spatial resolution \\
	\hline
	Commercial logs, survey data, real population & weekly, monthly, years
	& 1, 2, 5, 10 \\ 
\end{tabular}
\label{tab:x}
\end{table}

ABOVE WOULD BE BETTER AS SOME SORT OF GRAPHICAL ILLUSTRATION OF AGGREGATION
CONSEQUENCES, E.G. 
COMMERCIAL DATA SPARSITY, SURVEY DATA CONSISTENCY, REAL
POPULATION IS ALL. 

TEMPORALLY VARYING PROPORTIONS DIVIDED BY WEEK, MONTH, YEAR

CELLS AGGREGATED ACCORDING TO SIMULATION EXAMPLES !!!
