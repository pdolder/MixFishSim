...
\node [test, densely dotted] (t1) {if $spawn_{t}$};
\node [test, join, densely dotted] (t2) {if $wk_{t}$ != $wk_{t-1}$};
...
\caption{Overview Schematic of simulation model. The blue boxes indicate fleet
	...
	$tmax$ is the total number of tows, $wk$ is a weekly timestep, $day$ is
	a day timestep, $trip$ is a trip time step. \textcolor{red}{I NEED TO
		REDO THIS TO MAKE NOTATION MORE CONCISE AND CONSISTENT} } 
\label{fig:overview}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
of fishing events \replaced[id=PD]{as defined by the revenue gained}{in value}.
...
walk \todo{\added[id=JJ]{Correlated random walk of what}} function
...
commonly used in ecology to describe \added[id=PD]{optimal} animal
\deleted[id=PD]{movement which} search\replaced[id=PD]{ strategy}{ing} for
...
\deleted[id=PD]{location of} directional movement \citep{Codling2008}
\deleted[id=PD]{or uncorrelated}. \\
