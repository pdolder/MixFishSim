...
species-unselective gear. This can result in unintended, unwanted catch of low
quota or protected species and reducing these catches is crucial for
biological and economic sustainability of `mixed fisheries'. \\
%%
\replaced[id = PD]{If fisheries are to avoid unwanted catch,}{To implement
	effective spatial measures to reduce discards} \replaced[id = CM]{an in-depth}{a good} understanding of
...
distributions and movement, and how fishers interact with different fish
populations. Because data on fish location at high temporal and spatial
resolutions is expensive and difficult to collect proxies are usually inferred
from scientific surveys or commercial catches with limited spatial and temporal
resolution. \\ 
%%
To understand how data resolution impacts inference on mixed fisheries
interactions, we developed a highly resolved spatiotemporal \added[id=CM]{event-based} simulation model
...
on species targeting under an explore-exploit strategy. This is implemented via
a mix of correlated random walk movement (for exploration) and learned
behaviour (for exploitation) phases of the fisheries.  \\ 
%%
\replaced[id=CM]{Fifty}{We simulated 50} years of \added[id=CM]{sub-daily} fishing \added[id=CM]{was simulated} and used \deleted[id = CM]{the results}to draw inference on the
underlying community structures. We compared inference\added[id=CM]{s} \added[id=CM]{based on:}\deleted[id=CM]{from} commercial catch\added[id=CM]{,} \deleted[id=CM]{to}
a simulated fixed-site sampling \added[id=CM]{survey} design and the true underlying community. We i)
establish the potential limitations of fishery-dependent data in providing a
robust picture of spatiotemporal distributions; and then ii) simulated an area
closure based on areas defined from the different data sources at a range of
temporal and spatial resolutions. \deleted{We assessed the effectiveness of the closures
in reducing catches of a fish population. \\
%%
...
fine spatial and temporal scale.} We conclude from our \replaced[id=CM]{example framework application}{simulations} that
commercial data, while containing bias, provide a useful tool for managing
catches in mixed fisheries if applied at the correct spatiotemporal scale. [NEED A LITTLE MORE DETAIL OF THE RESULTS HERE - 2 MORE SENTENCES ON KEY TAKE-HOME RESULTS]\\
%%
\end{abstract}
...
\replaced[id=PD]{Adaptive spatial management strategies have}{Use of spatial
	management as a tools} been proposed as a way of reducing over-quota
discards \added[id=PD]{\citep{Holmes2011, Little2014, Dunn2014a}}.
\deleted[id=PD]{its} However, implementation is restricted by lack of knowledge
of fish and fishery spatiotemporal dynamics and understanding of the scale at
which processes become important for management. Understanding the correct
scale for spatial measures is crucial for implementing effective solutions
which  minimising economic impact \citep{Dunn2016}. For example, the problem
can be to identify a scale that promotes species avoidance for vulnerable or
...
spatial scales for fisheries closures has been a challenge in the past but is
crucial to their success \citep{Costello2010, Dunn2016}. Inference is hampered
where spatial information is coarse due to low resolution reporting of
fisheries catch which is aggregated across larger gridded areas
...
spatial management has been possible \citep[e.g.][]{Holmes2011}. However,
...
over-represented in \added[id=CM]{the} catch as fishers exploit areas of high abundance.
\deleted[id=PD]{ Further, fishers generally only recorded landings (not catch)
	on a daily basis\todo{\added[id=JJ]{Its not clear what the problem is:
			landings collected on daily basis or landings recorded
			rather than catch}}.  This leads to questions about the
...
\replaced[id=PD]{\replaced[id=CM]{To}{In order to} understand the effect of spatiotemporal
	aggregation of data and fishery targeting we ask two fundamental
	questions regarding inference derived from observational data:}{ and
	use the results}\deleted[id=PD]{ from the fishing model:}
...
process to define suitable habitat for different populations\replaced[id=PD]{.
	Precise locations of the fish being}{ and are} known
\added[id=PD]{directly} rather than inferred from sampling or commercial
catch\added[id=PD]{, we can use the population model to validate how inference
	from fisheries-dependent and fisheries independent sampling relates to
	the real community structure in a way we could not with real data}. We
ii) compare, at different spatial and temporal aggregations, the \replaced[id=CM]{`}{'}real
population' distributions to samples from fisheries-dependent and fisheries
independent catches to test if these are a true reflection of the relative
density of the populations. We then iii) simulate a fishery closure to protect
a species based on different spatial and
...
\replaced[id=PD]{An}{We developed and implemented a fully spatially explicit
	simulation model with a} \replaced{event-based simulation model of a
	hypothetical fishery was developed as a software package
	(`MixFishSim'). The modular approach enabled efficient computation by allowing
...
\replaced[id=PD]{P}{p}opulation dynamics operate on a daily
time-step\replaced[id=PD]{, while p}{. P}opulation movement occurs on a weekly
time-step, with the fishing module operating on a tow-by-tow basis (i.e.
multiple events a day).  \deleted[id=PD]{Here we describe each of the model
components; 1) Population dynamics, 2) Recruitment dynamics, 3) Population
movement dynamics, 4) fishery dynamics.}
...
\added[id=PD]{A daily time-step was chosen to discretise continuous population
	...
	timescale.} \deleted[id=PD]{Here,}Population biomass growth was
...
\begin{equation}
	\begin{split}
	B_{c,d+1} = &\\
	& (1 + \rho) B_{c,d} \cdot \mathrm{e}^{-Z_{c,d}} - \rho \cdot
	\mathrm{e}^{-Z_{c,d}} \hspace{2.9cm}
	\times \\  
	& (B_{c,d-1} \cdot \mathrm{e}^{-Z_{c,d-1}} + Wt_{R-1} \cdot \alpha_{d-1} \cdot
	R_{\tilde{y}(c,y,d-1)})
	\hspace{0.4cm} + \\
...
\begin{equation}
C_{c,d} = \frac{F_{c,d}}{F_{c,d} + M_{c,d}} \deleted[id=CM]{*} \added[id=CM]{\left(}1 - \mathrm{e}^{-(F_{c,d} + M_{c,d})}\added[id=CM]{\right)}
\cdot B_{c,d}
\end{equation}
...
\emph{MixFishSim}, it can be either as a stochastic Beverton-Holt stock-recruit
...
\replaced[id=CM]{characterise}{incorporate characterisation of} a \replaced[id=CM]{set}{number} of different fishing
...
\begin{equation}
	\replaced[id=CM]{Pr(C_{wk+1} = J | C_{wk} =I)}{Pr(J | I)} = \frac{\mathrm{e}^{-\lambda * d_{IJ}} \cdot
		(Hab_{J,p}^2 \cdot
		Tol_{J,p,wk})}{\sum\limits_{c=1}^{C}\mathrm{e}^{-\lambda * d} \cdot
...
standard deviation of the population temperature tolerance. \deleted[id=CM]{\\}

\added[id=PD]{The final combined process results in a population structure and
	movement pattern unique to each species, with population movement
	occurring on a weekly basis. The decision to model population movement
	on a weekly timescale was to reflect that fish tend to aggregate in
	species specific locations that have been observed to last around one
	to two weeks \citep{Poos2007}. Therefore this process approximated the
...
\replaced[id=CM]{F}{The f}leet dynamics can be broadly categorised into three components\replaced[id=CM]{:}{;} fleet
targeting - that determined the fleet catch efficiency and preference towards a
particular species; trip-level decisions, that determine\replaced[id=CM]{s}{d} the initial location
to be fished at the beginning of a trip; and within-trip decisions, that
determine\replaced[id=CM]{s}{d} movement from one fishing spot to another within a trip. Together,
these elements implement\deleted[id=CM]{ed} an explore-exploit type strategy for individual
...
\citep{VanPutten2012a}. Therefore in the simulations fleet dynamics \replaced[id=CM]{reflect}{are the
...
population $p$, and $Pr_{p}$ price of a population. All species prices were
kept the same across fleets and seasons. Here, when fishing is successful
vessels remain in a similar location and continue to exploit the local fishing
grounds. When unsuccessful, they move some distance away from the current
fishing location.  The movement distance retains some degree of stochasticity,
that can be controlled separately, but is determined by the
relationship: 
\begin{equation}
	StepL = \mathrm{e}^{log(\beta_{1}) + log(\beta_{2}) -
...
location choice of the fleet as a whole \citep{Rijnsdorp2000, Poos2007a}. \replaced[id=CM]{To}{In order to} account for this behaviour, the fishing sub-model operates spatially
...
the author\added{'}s github site (\url{www.github.com/pdolder/MixFishSim}).\\

\section{Parameterisation}
\added[id=CM]{We parameterise \emph{MixFishSim} to investigate the influence of data aggregation on spatial inference.}

\subsection{Population models}

We parametrised the simulation model for four \added[id=CM]{example} populations with different
demographics\replaced[id=CM]{,}{;} growth rates, natural mortality and recruitment parameters (Table
...
\added[id=CM]{learned} random walk process implies that within a fleet different vessels have
...
\subsection{\replaced[id=CM]{Emergent s}{S}imulation dynamics}
...
experience during the explor\replaced[id=CM]{atory}{e} phase of the fishery (\added[id=PD]{Figure}
...
\replaced[id=CM]{To}{In order to} answer this question we compare different spatial and temporal
aggregations of the \deleted[id = CM]{'}real population\deleted[id = CM]{'} distributions to:
...
there remains good consistency between the \deleted[id = CM]{'}real population\deleted[id = CM]{'} and the commercial
...
\deleted[id = CM]{'}real population\deleted[id = CM]{'}, the monthly aggregation captures the major patterns of
...
\deleted[id = CM]{'}real population\deleted[id = CM]{'}, though the species 1 (in red) is less well represented and
...
following which the fisheries \replaced[id=CM]{``}{"}adapted" to the closures and fishing mortality
...
based on the \replaced[id=CM]{``}{"}known" population distribution were most effective, with
...
protect a core are\added[id=CM]{a} of the habitat for the population, but this was achieved in
...
%% General intro
Our study \added{presents a new highly resolved fisheries simulation framework,} evaluates the importance of data scaling and considers potential bias
...
approaches \replaced[id=CM]{to}{In order to} maximise productivity while taking account of both the
...
dynamics in a hypothetical \replaced[id=CM]{`}{'}mixed fishery', allowing us to i) evaluate the
...
account \added[id=CM]{(Figures X,Y,Z)}. \\
...
capture some of the same spatial patterns as the \replaced{`}{'}real population', but missed
others due to gaps between survey stations limiting spatial and temporal
coverage \added[id=CM]{(Figures X,Y,Z)}. This provides a challenge when modelling unsampled areas in inferring
...
\ref{S-fig:1}, \ref{S-fig:3}). Our assumptions in paramet\added[id=CM]{e}rising the model
...
Other novel applications of our framework could be\replaced[id=CM]{:}{;} testing different survey
