...
#' \strong{catches_list}, which is a list of dataframes for recording the
#' vessels catches for each fleet.
...
init_fleet <- function(sim_init = NULL, n_fleets = 1, n_vessels = 1, VPT = NULL, Qs = NULL, step_params = NULL, past_knowledge = FALSE, past_year_month = FALSE, past_trip = FALSE, threshold = FALSE) {
	...
	params_lst <- lapply(n_fleets, function(x) {
	...
	catches_lst <- lapply(1:n_fleets, function(x) {
	catch.mat <- cbind(fleet = x, vessel = rep(1:n_vessels, each = nrow(catch.mat)), 
